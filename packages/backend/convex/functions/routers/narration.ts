...
export const prepare = action
  ...
  .action(async ({ ctx, input }): Promise<{ status: 'scheduled' }> => {
    ...
    if (record && ((record.narrationText || record.content) !== input.text || record.voice !== input.voice)) throw new Error('Recording content does not match.');
    // Reading a recording cannot create or retry paid work. Owners use listening.control.
    if (record) return { status: 'scheduled' };
...
export const page = query
  .input(z.object({ contentDigest: z.string().regex(/^[0-9a-f]{64}$/), voice: z.string().min(1).max(100), from: z.number().int().min(0).max(1000), completedManifest: z.boolean().optional(), recordingId: z.string().optional(), ownerToken: z.string().optional() }))
  .query(async ({ ctx, input }): Promise<NarrationPage> => {
    ...
    const rows = await db.query('narrationSections').withIndex('by_jobId_and_index', q => q.eq('jobId', job._id).gte('index', input.from)).take(input.completedManifest && job.status === 'done' ? 1000 : 4);
    ...
    return { status: job.status === 'cancelled' ? 'failed' : job.status, total: job.total, completed: job.completed, error, sections };
  });
